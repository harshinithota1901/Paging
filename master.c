...
static pid_t childpids[MAX_USERS];  
...
void pcb_release(struct process * procs, const unsigned int pi){
  ...
  //clearing frames, used by that user
...
static int master_initialize()
{
  ...
  //initialization of page and frame tables
	for(i=0; i < MAX_USERS; i++){
...
//Sending a message to user process. Buffer must be filled!
static int send_msg(struct msgbuf *m)
{
	m->from = getpid();	//mark who sends the message
