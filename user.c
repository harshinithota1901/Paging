...
{
	key_t key = ftok(FTOK_SHM_PATH, FTOK_SHM_KEY);  //getting a key for the shared memory
	if(key == -1){
		perror("ftok");
		return -1;
	}
...
static void msg_reference(struct msgbuf *msg, enum request d, const int arg_m){
	...
	if(arg_m == 0){
		...
		//finding the page index using weights
		...
		//calculating page and generate random offset
...
int main(const int argc, char * const argv[]){
  ...
  pcb = &shmp->procs[my_index];
	...
	//initializing the rand functions
	srand(getpid());
	srand48(getpid());
	...
	while(terminate_me == 0){
		...
		//checking for termination, by reading invalid address
	...
	//terminating by writing to invalid address
